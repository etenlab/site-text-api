package org.eten.databaseapi.component
...
class DatabaseVersionControl(
  ...
  fun initDatabase() {
    if (!isDbInit()) {
      toVersion1()
      if (app_config.load_db_version > 1) {
//        updateSchemaIdempotent(app_config.load_db_version)
      }
//      updateHistoryTables()
//      updatePeerTables()
...
//  }
  ...
  private fun toVersion1() {
    ...
    // admin
    runSqlFile("sql/v1.schema.sql")

//    // prep and bootstrap the db
//    val pash = util.encoder.encode(app_config.cordAdminPassword)
//
...
//    this.loadTranslations()
  ...
  private fun getSchemaVersion(): Int {
    return reader_jdbc.jdbcTemplate.queryForObject(
        """
            select version 
  ...
  private fun setVersionNumber(newVersion: Int) {
    writer_jdbc.jdbcTemplate.update(
        """
            insert into database_version_control(version, started, finished)
